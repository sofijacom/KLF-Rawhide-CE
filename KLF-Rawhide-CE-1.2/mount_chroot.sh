#!/bin/sh
# mount_chroot.sh
# Creation Date: 23May2019 Revision Date: 08Aug2023
...
progname="mount_chroot.sh"; version="3.0.1"; revision="-rc1"
...
mv "${chroot_dir}"/etc/resolv.conf "${chroot_dir}"/etc/resolv.confORIG && cp /etc/resolv.conf "${chroot_dir}"/etc/resolv.conf  # changing etc/resolv.conf causes problems sometimes. For example, systemd distros want to auto-create resolv.conf as a symlink 

# chroot into firstrib rootfs and execute a /bin/sh process from that root filesystem
chroot "${chroot_dir}" sh
rm -f "${chroot_dir}"/etc/resolv.conf && mv "${chroot_dir}"/etc/resolv.confORIG "${chroot_dir}"/etc/resolv.conf  # put back resolv.conf to its original form
